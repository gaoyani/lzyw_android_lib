...
public class FavoriteStoreListTask extends AsyncTask<String, Void, Integer> {
	private Context context = null;
	private Handler handler = null;
	private Message message = null;
	private int startIndex = 0;

	public FavoriteStoreListTask(Context context, Handler handler, int startIndex) {
	...
	protected Integer doInBackground(String... params) {
		...
		try {
			...
			HttpPost httpGet = new HttpPost(UrlConstant.FAVORITE_STORE_LIST_URL);
			...
			if (code == HttpStatus.SC_OK) {
				...
				if (jsonArray.length() != 0) {
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						Data.favoriteStoreList.add(storeInfo);
					}
				} else {
