...
public class RoomReservationTask extends AsyncTask<String, Void, Integer> {
	...
	public RoomReservationTask(Context context, Handler handler, ReservationInfo reservationInfo) {
	...
	protected Integer doInBackground(String... params) {
		String url = UrlConstant.ADD_ROOM_ORDER_URL;
		message = new Message();
		...
		try {
			...
			param.put("type", reservationInfo.type);
			param.put("other_info", reservationInfo.otherInfo);
			JSONArray timeArray = new JSONArray();
			for (int i = 0; i < reservationInfo.timeList.size(); i++) {
				timeArray.put(reservationInfo.timeList.get(i));
			}
			param.put("time_list", timeArray);
